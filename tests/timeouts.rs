...
async fn client_timeout() {
    ...
    let client = dusks_reqwest::Client::builder()
...
async fn request_timeout() {
    ...
    let client = dusks_reqwest::Client::builder().build().unwrap();
...
async fn response_timeout() {
    ...
    let server = server::http(move |_req| {
        async {
            ...
            let body = dusks_reqwest::Body::wrap_stream(futures_util::stream::once(async {
...
async fn read_timeout_allows_slow_response_body() {
    ...
    let server = server::http(move |_req| {
        async {
            ...
            let body = dusks_reqwest::Body::wrap_stream(slow);
...
fn timeout_closes_connection() {
    ...
    // close too early.
    let client = dusks_reqwest::blocking::Client::builder()
        .timeout(Duration::from_millis(500))
        .build()
        .unwrap();
...
fn timeout_blocking_request() {
    ...
    // Make Client drop *after* the Server, so the background doesn't
    // close too early.
    let client = dusks_reqwest::blocking::Client::builder().build().unwrap();
...
fn write_timeout_large_body() {
    ...
    let err = client
        .post(&url)
        .body(dusks_reqwest::blocking::Body::sized(cursor, len as u64))
        .send()
        .unwrap_err();
